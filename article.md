...
import pandas as pd
    ...
    # Sample data
dates = pd.date_range(start='2025-01-01', periods=24, freq='M') values = [10, 12, 15, 14, 11, 13, 16, 14, 12, 15, 17, 19, 18, 16, 14, 12, 14, 17, 15, 13, 11, 14, 16, 18]

    # Convert to a pandas Series
ts = pd.Series(values, index=dates) ts.head()
...
date_range = pd.date_range(start="2024-01-01", periods=48, freq="H") data = { 'temperature': np.random.normal(35, 2, len(date_range)), 'weight': 50 + np.cumsum(np.random.normal(0.2, 0.1, len(date_range))), 'bee_traffic': np.sin(np.linspace(0, 3*np.pi, len(date_range))) * 50 + 200 } df = pd.DataFrame(data, index=date_range) df.index.name = 'time' df.head()
